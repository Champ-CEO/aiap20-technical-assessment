...
class PipelineUtils:
    ...
    def split_data(self, 
        ...
        try:
            ...
            stratify = stratify if stratify is not None else self.config['stratify_target']
            random_state = random_state or self.config['random_state']
            
            # Validate inputs
            if target_column not in df.columns:
                raise PipelineUtilsError(f"Target column '{target_column}' not found in data")
            
            if test_size + validation_size >= 1.0:
                raise PipelineUtilsError("Combined test_size and validation_size must be < 1.0")
    ...
    def optimize_memory_usage(self, df: pd.DataFrame, aggressive: bool = False) -> pd.DataFrame:
        """
        ...
        try:
            ...
            logger.info(f"Starting memory optimization. Initial memory: {initial_memory:.2f} MB")
            ...
            logger.info(f"Memory savings: {memory_saved:.2f} MB ({memory_reduction_percent:.1f}%)")
    ...
    def monitor_performance(self, operation_name: str, func, *args, **kwargs):
        ...
        try:
            ...
            if hasattr(result, '__len__') and len(result) > 1000:
                records_per_second = len(result) / execution_time if execution_time > 0 else float('inf')
                if records_per_second >= self.config['performance_standard']:
                    logger.info(f"✅ Performance standard met: {records_per_second:,.0f} records/second")
                else:
                    logger.warning(f"⚠️ Performance below standard: {records_per_second:,.0f} records/second")
            ...
            logger.error(f"Performance monitoring failed for '{operation_name}': {str(e)}")
            raise
    
    def handle_data_access_error(self, error: Exception, file_path: str, retry_count: int = 3) -> Optional[pd.DataFrame]:
        """
    ...
    def _validate_stratification(self, original_df: pd.DataFrame, splits: Dict[str, pd.DataFrame], target_column: str) -> None:
        """Validate that stratification preserved target distribution."""
        original_dist = original_df[target_column].value_counts(normalize=True).sort_index()
        
        for split_name, split_df in splits.items():
            split_dist = split_df[target_column].value_counts(normalize=True).sort_index()
            
            # Check if distributions are similar (within 5% tolerance)
            max_diff = abs(original_dist - split_dist).max()
            if max_diff > 0.05:
                logger.warning(f"Stratification may not be perfect for {split_name} split. Max difference: {max_diff:.3f}")
            else:
                logger.info(f"✅ Stratification validated for {split_name} split")
    
    def _log_split_summary(self, splits: Dict[str, pd.DataFrame], split_info: Dict[str, Any], split_time: float) -> None:
        ...
        for split_name, split_df in splits.items():
            ratio = split_info.get(f'{split_name}_ratio', len(split_df) / split_info['total_records'])
            logger.info(f"{split_name.capitalize()} set: {len(split_df):,} records ({ratio:.1%})")
